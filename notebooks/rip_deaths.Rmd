...
# covid_df <- covid_df %>% replace(is.na(.), 0)
```
...
# 
# daily_covid_deaths = covid_df  %>% select(Date, TotalCovidDeaths, ToatlConfirmedCovidCases)
...
# covid_rip_join <- left_join(daily_covid_deaths, daily_rip_deaths, by = c("Date" = "published"))
```


#### Add new column with accumulated deaths so far per date
